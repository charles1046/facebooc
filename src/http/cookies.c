...
{
    return strlen(t->key) + strlen(t->value) + strlen(t->expires) +
           strlen(t->max_age) + strlen(t->domain) + strlen(t->path) +
...
{
    if (unlikely(!cs || !key || !*key ||
                 !value))  // value could be "" empty string
        return cs;
...
char *Cookie_to_string(const Cookie *c)
{
    ...
    snprintf(str, cur_len + 2, "%s=%s%s%s%s%s%s%s", t.key, t.value, t.expires,
...
void Cookies_print(const Cookies *cs)
{
    ...
    list_for_each(cur, &cs->list)
    {
        ...
        const Cookie_to_string_template__ t = {
            ...
            .path =
                cur_c->path ? cur_c->path : "; Path=/",  // Default path is root
            .secure = cur_c->secure ? "; Secure" : "",
            .httponly = cur_c->httponly ? "; HttpOnly" : "",
        };
        fprintf(stderr, "%s=%s%s%s%s%s%s%s\n", t.key, t.value, t.expires,
                t.max_age, t.domain, t.path, t.secure, t.httponly);
    }
}
#endif
