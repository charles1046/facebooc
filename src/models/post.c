...
Post *postGetById(sqlite3 *DB, int id)
{
    ...
    Basic_string *body =
        Basic_string_init((const char *) sqlite3_column_text(statement, 3));
    newline_to_br(body);

    post = postNew_move(sqlite3_column_int(statement, 0),
...
Posts *postGetLatest(sqlite3 *DB, int accountId, int page)
{
    ...
    while (sqlite3_step(statement) == SQLITE_ROW) {
        Basic_string *tmp_body =
            Basic_string_init((const char *) sqlite3_column_text(statement, 3));
        newline_to_br(tmp_body);

        Posts *new_ps = Posts_new();
        new_ps->p = postNew_move(sqlite3_column_int(statement, 0),
                                 sqlite3_column_int(statement, 1),
