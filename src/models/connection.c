...
Connection* connectionCreate(sqlite3* DB, int account1Id, int account2Id) {
	...
	if(sqlite3_step(statement) == SQLITE_DONE) {
		connection = connectionNew(sqlite3_last_insert_rowid(DB), t, account1Id, account2Id);
	}
...
Connection* connectionGetByAccountIds(sqlite3* DB, int account1Id, int account2Id) {
	...
	connection = connectionNew(sqlite3_column_int(statement, 0), sqlite3_column_int(statement, 1),
