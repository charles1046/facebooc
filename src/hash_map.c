...
void *Hash_map_get(const Hash_map *map, const char *key)
{
    ...
    while (map->entries[index]) {
        ...
        ++index;
        if (unlikely(index >=
                     map->cap))  // Use branching to enhance performance
...
static inline _Bool hash_map_move_entry(SPair **new_entries,
                                        size_t cap,
                                        SPair *p)
{
...
static inline _Bool hash_map_set_entry(SPair **entries,
                                       size_t cap,
                                       const SPair *p)
{
    size_t index = get_index(cap, p->key);
    ...
    while (entries[index]) {
        if (unlikely(p == entries[index]))
            ...
            SPair_delete(entries[index]);  // Found key (it already exists),
                                           // overwrite pair pointer.
            break;
        }
